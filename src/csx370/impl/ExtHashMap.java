 ...
 */
public class ExtHashMap<K, V> extends AbstractMap<K, V> implements Serializable, Cloneable, Map<K, V> {
	...
	public V get(Object key) {
		...
		//Iterate through filled portion of key array 'key'
		for (int j = 0; j < b.nKeys; j++) {
			//Item matches 'key', return value at key index from 'value' array
			if (b.key[j] == key) {
				return b.value[j];
			}
		}
	...
	public V put(K key, V value) {
		...
		//Generate directory buckets up to and including 'i'
		if (i >= dir.size()) {
...
//			out.println ("key = " + j + " value = " + dir.get (j));
//		} // for
		
		System.out.println(i);
		Bucket b = dir.get(i);
		System.out.println("bucket:" + b);
		System.out.println("bucket nKeys: " + b.nKeys);
